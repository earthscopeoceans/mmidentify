...
integer :: jdebug=0
real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(102,17),depth(17)
...
real function PKPabtime(dist,dep)

! computes travel time of PKPab wave, 143<dist<178 deg, 0<dep<700
! returns -12345. if dist or dep outside range

real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(36,8),depth(8)
...
real function PKPbctime(dist,dep)

! computes travel time of PKPbc wave, 143<dist<155 deg, 0<dep<700
! returns -12345. if dist or dep outside range

real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(13,8),depth(8)
...
real function PKPdftime(dist,dep)

! computes travel time of PKPdf wave, 113<dist<180 deg, 0<dep<700
! returns -12345. if dist or dep outside range

real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(68,8),depth(8)
...
real function Stime(dist,dep)

! computes travel time of S or Sdiffracted wave, 0<dist<125 deg, 0<dep<700
! returns -12345. if dist or dep outside range
...
real function smallPtime(dist,dep)
...
real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(100,0:12),depth(0:12)
...
implicit none
real*4 :: dist,depth,del,rad,t1,t2

del=int(dist)                  ! lower bound for interpolation
del=max(1.0,del)
rad=57.29578
if(del<113) then
  t2=Ptime(del+1.0,depth)
  t1=Ptime(del,depth)
else
  t2=PKPdftime(del+1.0,depth)
  t1=PKPdftime(del,depth)
endif  
if(t1<0. .or. t2<0.) then
  slw=-12345.
  return
endif
slw=rad*(t2-t1)           ! slowness in sec/radian
if(slw<0.1) slw=0.1

return
end function

real function slwS(dist,depth)

! find slowness for S at distance dist (degrees) for sourc at depth (km)
...
del=max(1.0,del)
rad=57.29578
t2=Stime(del+1.0,depth)
t1=Stime(del,depth)
if(t1<0. .or. t2<0.) then
  slwS=-12345.
  return
endif  
slwS=rad*(t2-t1)           ! slowness in sec/radian

return
end function


real function slwsmallP(dist,depth)

! find slowness at distance dist (degrees) for sourc at depth (km)
! Assumes pPKP DF core phase if del>113, else pP or pPdiff
...
rad=57.29578
if(del<113) then
  t2=Ptime(del+1.0,depth)+smallPtime(del+1.0,depth)
  t1=Ptime(del,depth)+smallPtime(del,depth)
else if(depth<35.) then         ! treat like PKP surface source  
  t2=PKPdftime(del+1.0,0.)
  t1=PKPdftime(del,0.)
else
  t2=0.
  t1=0.
  ! print *, 'WARNING: Cannot handle depth for PKP, assume slw=0.1'
endif  
if(t1<0. .or. t2<0.) then
  slwsmallP=-12345.
  return
endif
slwsmallP=rad*(t2-t1)               ! slowness in sec/radian
if(slwsmallP<0.1) slwsmallP=0.1
...
real function Pdiftime(dist,dep)
...
integer :: jdebug=0
real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(26,17),depth(17)
...
real function Sdiftime(dist,dep)
...
real function PPtime(dist,dep)

! computes travel time of PP wave, 28<dist<180 deg, 0<dep<700
! returns -12345. if dist or dep outside range

real :: dist,dep     ! distance and depth of the earthquake
real :: dmin,dmax,time(153,8),depth(8)
...
t1=time(i0,j1)+x*(time(i1,j1)-time(i0,j1))
PPtime=t0+y*(t1-t0)
!write(13,*) 'PP dep,dist:',dep,dist,'depth(j0)=',depth(j0),'t=',PPtime
!write(13,*) 'i0,j0=',i0,j0,' d0,x,y=',d0,x,y
!write(13,*) 'times:',time(i0,j0),time(i1,j0),time(i0,j1),time(i1,j1)
!write(13,*) 'min, sec:',int(time(i0,j0)/60.),time(i0,j0)-int(time(i0,j0)/60.)*60.
